...
GFFX_API gffx_status GFFX_CALL gffx_mesh_face_geometry_backward(
    ...
    if (context != NULL && context->struct_size >= sizeof(*context) &&
        ...
        if (operations->mesh_face_geometry_backward == NULL) {
            return gffx_internal_fail(
                diagnostic, GFFX_STATUS_UNSUPPORTED,
                "the CUDA provider does not implement this operation");
        }
        return operations->mesh_face_geometry_backward(
            vertices, faces, eps, grad_unit_normals, grad_areas, context, grad_vertices,
